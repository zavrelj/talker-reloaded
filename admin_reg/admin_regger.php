				...
				if(mysql_num_rows($isRegistered)) {
					...
					header("location: newuser.php?note=801");
  					...
  					if ($regfpassword == $reg2fpassword) {
						...
						//echo "neni registrovan - muzem registrovat";
						...
						if (ereg("(^[[:alnum:]]+$)", $regflogin)) {
							...
							//echo "jmeno proslo kontrolou regexpu";


							_oxMod("INSERT INTO $TABLE_USERS VALUES (LAST_INSERT_ID(),'$regflogin','$hash_password',$now,$now,0,0,0,'','','',1200,'','','','','','','','','','','','','','','','',0,0,0,0,'','default',10,'','','','','','','','','','','available','1',0,'','',1,'','cz')");
